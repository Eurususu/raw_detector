 ...
 * Last modified : 2020-06-24 09:17
...
class NvEncoderNotify
{
public:
    virtual ~NvEncoderNotify(){};
    virtual void OnVideoEncodedData(char* pData, int32_t nLen, uint64_t ulTimestamp, int32_t nWidth, int32_t nHeight, bool bKeyFrame = false) = 0;
};
...
public:
    ...
    */
    ...
    char* GetNextOutputFrame()
	{
		// return m_pOutputQueue->Next();
        return nullptr;
	}
    char* GetCurrentOutputFrame()
	{
		// return m_pOutputQueue->Current();
        return nullptr;
	}
	void WriteEnd()
	{
		// m_pOutputQueue->WriteEnd();
	}

    void SetEncodeConfig(int nBitRate, int nRateModel);
    /**
    ...
    */
    ...
    void EncodeFrame(uint32_t nWidth, uint32_t nHeight, uint64_t& iTimestamp, NV_ENC_PIC_PARAMS *pPicParams = nullptr);
    ...
    */
    ...
    void EndEncode(uint64_t& iTimestamp);
    ...
    */
    ...
    int GetRateMode() const {return m_nRateModel;}
...
protected:
    ...
    */
    NvEncoder(NV_ENC_DEVICE_TYPE eDeviceType, void *pDevice, uint32_t nWidth, uint32_t nHeight,
        NV_ENC_BUFFER_FORMAT eBufferFormat, uint32_t nOutputDelay, NvEncoderNotify& rNotify, bool bMotionEstimationOnly, bool bOutputInVideoMemory = false);
    ...
    */
    void* GetCompletionEvent(uint32_t eventIdx) { return ((int)m_vpCompletionEvent.size() == m_nEncoderBuffer) ? m_vpCompletionEvent[eventIdx] : nullptr; }
...
private:
    ...
    */
    ...
    void GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, uint32_t nWidth, uint32_t nHeight,  uint64_t& iTimestamp, bool bOutputDelay);
...
private:
    ...
    NvEncoderNotify& m_rNotify;
    int32_t             m_nBitRate{8000};
    int32_t             m_nRateModel{0};
};
