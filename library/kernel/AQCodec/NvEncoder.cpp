 ...
 * Last modified : 2020-08-12 14:22
...
NvEncoder::NvEncoder(NV_ENC_DEVICE_TYPE eDeviceType, void *pDevice, uint32_t nWidth, uint32_t nHeight, NV_ENC_BUFFER_FORMAT eBufferFormat,
                            uint32_t nExtraOutputDelay, NvEncoderNotify& rNotify, bool bMotionEstimationOnly, bool bOutputInVideoMemory) :
...
void NvEncoder::SetEncodeConfig(int BitRate, int RateModel)
{
    m_nBitRate = BitRate;
...
void NvEncoder::CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS* pIntializeParams, GUID codecGuid, GUID presetGuid)
{
    ...
    {
        pIntializeParams->encodeConfig->rcParams.rateControlMode = NV_ENC_PARAMS_RC_CBR_HQ;
        ...
        && pIntializeParams->presetGUID != NV_ENC_PRESET_LOSSLESS_HP_GUID)
    {
        ...
        {
            pIntializeParams->encodeConfig->rcParams.averageBitRate = m_nBitRate * 1024;
            pIntializeParams->encodeConfig->rcParams.maxBitRate = m_nBitRate * 1024;
            // pIntializeParams->encodeConfig->rcParams.enableMaxQP = 1;
            // pIntializeParams->encodeConfig->rcParams.maxQP = {45, 45 ,37};
            // pIntializeParams->encodeConfig->rcParams.enableMinQP = 1;
            // pIntializeParams->encodeConfig->rcParams.minQP = {22, 23, 22};
        }
...
void NvEncoder::EncodeFrame(uint32_t nWidth, uint32_t nHeight, uint64_t& iTimestamp, NV_ENC_PIC_PARAMS *pPicParams)
{
    if (!IsHWEncoderInitialized())
    {
        NVENC_THROW_ERROR("Encoder device not found", NV_ENC_ERR_NO_ENCODE_DEVICE);
    ...
    {
        m_iToSend++;
        GetEncodedPacket(m_vBitstreamOutputBuffer, nWidth, nHeight, iTimestamp, true);
    }
    else
    {
        NVENC_THROW_ERROR("nvEncEncodePicture API failed", nvStatus);
...
void NvEncoder::EndEncode(uint64_t& iTimestamp)
{
    if (!IsHWEncoderInitialized())
    {
        NVENC_THROW_ERROR("Encoder device not initialized", NV_ENC_ERR_ENCODER_NOT_INITIALIZED);
    ...
    //GetEncodedPacket(m_vBitstreamOutputBuffer, iTimestamp, false);
}

void NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, uint32_t nWidth, uint32_t nHeight, uint64_t& iTimestamp, bool bOutputDelay)
{
    int iEnd = bOutputDelay ? m_iToSend - m_nOutputDelay : m_iToSend;
    for (; m_iGot < iEnd; m_iGot++)
    {
        ...
        NVENC_API_CALL(m_nvenc.nvEncLockBitstream(m_hEncoder, &lockBitstreamData));
		uint8_t *pData = (uint8_t *)lockBitstreamData.bitstreamBufferPtr;
		uint32_t iFrameDataSize = (uint32_t)lockBitstreamData.bitstreamSizeInBytes;
		uint32_t iFrameSize = iFrameDataSize + sizeof(uint64_t);

        bool bKeyFrame = false;
        if (lockBitstreamData.pictureType == NV_ENC_PIC_TYPE_I || lockBitstreamData.pictureType == NV_ENC_PIC_TYPE_IDR)
        {
            bKeyFrame = true;
        }        

        m_rNotify.OnVideoEncodedData((char*)pData, iFrameDataSize, iTimestamp, nWidth, nHeight, bKeyFrame);

		// char* pWritePos = m_pOutputQueue->GetPushBuf(iFrameSize + sizeof(uint32_t));
		// memcpy(pWritePos, &iFrameSize, sizeof(uint32_t));
		// memcpy(pWritePos + sizeof(uint32_t), &iTimestamp, sizeof(uint64_t));
		// memcpy(pWritePos + sizeof(uint32_t) + sizeof(uint64_t), pData, iFrameDataSize);
...
bool NvEncoder::Reconfigure(const NV_ENC_RECONFIGURE_PARAMS *pReconfigureParams)
{
    ...
    m_nMaxEncodeWidth = m_initializeParams.maxEncodeWidth;
    m_nMaxEncodeHeight = m_initializeParams.maxEncodeHeight;
    m_nBitRate = m_initializeParams.encodeConfig->rcParams.averageBitRate;
