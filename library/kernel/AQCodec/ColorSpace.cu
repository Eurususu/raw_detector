/**********************************************************
 * Author        : Peng Xuejian
 * Email         : pengxuejian@aliyun.com
 * Last modified : 2020-06-10 16:58
 * Filename      : ColorSpace.cu
 * Description   : 
 * *******************************************************/
/*
...
inline void GetConstants_(int iMatrix, float &wr, float &wb, int &black, int &white, int &max) {
    // Default is BT709
...
void SetMatYuv2Rgb(int iMatrix) {
    float wr, wb;
    int black, white, max;
    GetConstants_(iMatrix, wr, wb, black, white, max);
    float mat[3][3] = {
...
__device__ inline Rgb YuvToRgbForPixel(YuvUnit y, YuvUnit u, YuvUnit v, uint8_t * gamaTransLut = NULL, float* YUVDiff = NULL) {
	//if(gamaTransLut!= NULL) y = (YuvUnit)gamaTransLut[(int)y];
    const int 
        low = 1 << (sizeof(YuvUnit) * 8 - 4),
        mid = 1 << (sizeof(YuvUnit) * 8 - 1);
	/*if(YUVDiff != NULL){
		int16_t ty = y * (1 + YUVDiff[0]);
		if(ty > 0 && ty < 255) {
			y = (YuvUnit)ty;
		}else{
			if(ty <= 0){
...
__global__ static void YuvToRgbKernel(uint8_t *pYuv, int nYuvPitch, uint8_t *pRgb, int nRgbPitch, int nWidth, int nHeight, uint8_t * gamaTransLut = NULL, float* YUVDiff = NULL) {
    ...
    *(RgbIntx2 *)pDst = RgbIntx2 {
        YuvToRgbForPixel<Rgb>(l0.x, ch.x, ch.y, gamaTransLut, YUVDiff).d,
        YuvToRgbForPixel<Rgb>(l0.y, ch.x, ch.y, gamaTransLut, YUVDiff).d,
    };
    *(RgbIntx2 *)(pDst + nRgbPitch) = RgbIntx2 {
        YuvToRgbForPixel<Rgb>(l1.x, ch.x, ch.y, gamaTransLut, YUVDiff).d, 
        YuvToRgbForPixel<Rgb>(l1.y, ch.x, ch.y, gamaTransLut, YUVDiff).d,
    };
}
...
void Nv12ToColor32(uint8_t *dpNv12, int nNv12Pitch, uint8_t *dpBgra, int nBgraPitch, int nWidth, int nHeight, uint8_t* gamaTransLut, float* YUVDiff, int iMatrix) {
    SetMatYuv2Rgb(iMatrix);
    YuvToRgbKernel<uchar2, COLOR32, uint2>
        <<<dim3((nWidth + 63) / 32 / 2, (nHeight + 3) / 2 / 2), dim3(32, 2)>>>
        (dpNv12, nNv12Pitch, dpBgra, nBgraPitch, nWidth, nHeight, gamaTransLut, YUVDiff);
}
...
template <class COLOR32>
void P016ToColor32(uint8_t *dpP016, int nP016Pitch, uint8_t *dpBgra, int nBgraPitch, int nWidth, int nHeight, uint8_t* gamaTransLut, float* YUVDiff, int iMatrix) {
    SetMatYuv2Rgb(iMatrix);
    YuvToRgbKernel<ushort2, COLOR32, uint2>
        <<<dim3((nWidth + 63) / 32 / 2, (nHeight + 3) / 2 / 2), dim3(32, 2)>>>
        (dpP016, nP016Pitch, dpBgra, nBgraPitch, nWidth, nHeight, gamaTransLut, YUVDiff);
}
...
template void Nv12ToColor32<BGRA32>(uint8_t *dpNv12, int nNv12Pitch, uint8_t *dpBgra, int nBgraPitch, int nWidth, int nHeight, uint8_t* gamaTransLut, float* YUVDiff, int iMatrix);
template void Nv12ToColor32<RGBA32>(uint8_t *dpNv12, int nNv12Pitch, uint8_t *dpBgra, int nBgraPitch, int nWidth, int nHeight, uint8_t* gamaTransLut, float* YUVDiff,  int iMatrix);
...
template void P016ToColor32<BGRA32>(uint8_t *dpP016, int nP016Pitch, uint8_t *dpBgra, int nBgraPitch, int nWidth, int nHeight, uint8_t* gamaTransLut, float* YUVDiff, int iMatrix);
template void P016ToColor32<RGBA32>(uint8_t *dpP016, int nP016Pitch, uint8_t *dpBgra, int nBgraPitch, int nWidth, int nHeight, uint8_t* gamaTransLut, float* YUVDiff, int iMatrix);
